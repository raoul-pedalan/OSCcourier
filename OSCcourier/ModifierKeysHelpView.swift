...
struct ModifierKeysHelpView: View {
    ...
    private let entries: [Entry] = [
        ...
        Entry(keys: "⇧ drag", action: "Constrain the move to one axis (whichever moved further)",
              context: "Dragging a point"),
        Entry(keys: "⌥ hover", action: "Switch to “Duplicate track”",
              context: "Hovering the Delete-track button in a track header"),
        Entry(keys: "⌥ click", action: "Edit the grid's offset (Φ) and step (T)",
              context: "Clicking the Grid button in the command bar"),
        Entry(keys: "⌥ arrow", action: "Nudge by 10 pixels instead of 1",
              context: "Nudging the current selection with arrow keys"),
