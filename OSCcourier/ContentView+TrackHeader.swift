...
// The left-hand header column of a single track row: name (double-click to
// rename), mute/clear/duplicate/delete buttons, fold triangle, reorder
// handle, and (curve/step tracks) the amplitude range + quantization tick
// labels. Split out of `body`'s per-track ForEach verbatim — no logic
// changes. Paired with ContentView+TrackContent.swift via trackRow(index:)
// in ContentView+TrackRow.swift.
extension ContentView {
    func trackHeaderColumn(index: Int) -> some View {
